...
class HomeScreen(Screen):
    ...
    def _build_ui(self):
        ...
        # ── Search area ──
        search_box = self._build_search_area()
    ...
    def _build_header(self):
        header = BoxLayout(
            ...
            size_hint_y=None,
            height=dp(130),
            padding=[dp(PAD), dp(30), dp(PAD), dp(10)],
        )
    ...
    def _build_search_area(self):
        ...
        self._filter_all = self._make_filter_btn("All", True,
                                                  lambda _: self._set_filter(None))
        self._filter_disease = self._make_filter_btn("Diseases", False,
                                                      lambda _: self._set_filter("disease"))
        self._filter_plant   = self._make_filter_btn("Plants", False,
                                                      lambda _: self._set_filter("plant"))
    ...
    def _do_search(self, query):
        def _bg():
            results = self.db.search(query, top_k=12,
                                     doc_type=self._active_filter)
    ...
    def _load_initial(self, *_):
        def _bg():
            ...
            Clock.schedule_once(
                lambda _: self._render_browse(diseases, plants), 0
            )
        threading.Thread(target=_bg, daemon=True).start()
    ...
    def _render_browse(self, diseases, plants):
        self._results_container.clear_widgets()
        ...
        if self._active_filter != "plant":
            self._results_container.add_widget(
                self._section_label("Common Diseases")
            )
        ...
        if self._active_filter != "disease":
            self._results_container.add_widget(
                self._section_label("Medicinal Plants")
            )
