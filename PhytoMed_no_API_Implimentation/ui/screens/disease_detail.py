...
Shows: name, description, symptoms chips, prevention,
       and expandable plant remedy cards.
"""
...
class DiseaseDetailScreen(Screen):
    ...
    def _render(self, data):
        ...
        if data.get("description"):
            ...
            desc_lbl = BodyLabel(text=data["description"])
            self._content.add_widget(desc_lbl)
        ...
        if data.get("prevention"):
            ...
            prev_lbl = BodyLabel(text=data["prevention"])
            self._content.add_widget(prev_lbl)
        ...
        if plants:
            self._content.add_widget(
                SectionHeader(
                    title=f"Medicinal Remedies ({len(plants)})",
                    icon="🌿",
                    icon_color=SUCCESS
                )
            )
            ...
            for plant in plants:
                card = PlantRemedyCard(
                    plant_data=plant,
                    on_view_plant=self._open_plant
                )
            ...
            self._content.add_widget(
                EmptyState(
                    icon="🔎",
                    title="No plant remedies recorded",
