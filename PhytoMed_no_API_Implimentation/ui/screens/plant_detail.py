...
Shows: name, latin name, description, location, medicinal use,
       application steps, precautions, toxicity, treats (disease list).
"""
...
class PlantDetailScreen(Screen):
    ...
    def _render(self, data):
        ...
        icon_lbl = Label(text="🌿", font_name=FONT_EMOJI, font_size=dp(30),
                         size_hint=(None, 1), width=dp(40))
        name_col = BoxLayout(orientation="vertical", spacing=dp(2))
        n_lbl = Label(
            text=en_name,
            font_size=dp(FONT_H2), bold=True,
            color=WHITE,
            halign="left", valign="bottom"
        )
        ...
        if loc or cam:
            self._content.add_widget(SectionHeader(title="Geographic Location", icon="📍",
                                                     icon_color=INFO))
            if loc:
        ...
        if med:
            self._content.add_widget(SectionHeader(title="Medicinal Use", icon="💊",
                                                     icon_color=SUCCESS))
        ...
        if steps:
            self._content.add_widget(SectionHeader(title="How to Use", icon="📋",
        ...
        if prec:
            self._content.add_widget(
                SectionHeader(title="Precautions", icon="⚠️", icon_color=WARNING)
            )
            prec_lbl = BodyLabel(text=prec, color=WARNING)
            self._content.add_widget(prec_lbl)

        # ── Toxicity ──
        tox = data.get("toxicity", "")
        if tox:
            self._content.add_widget(
                SectionHeader(title="Toxicity", icon="☠️", icon_color=DANGER)
            )
            tox_lbl = BodyLabel(text=tox, color=DANGER)
            self._content.add_widget(tox_lbl)

        # ── Diseases it treats ──
        if treats:
            self._content.add_widget(
                SectionHeader(
                    title=f"Diseases Treated ({len(treats)})",
                    icon="💊",
                    icon_color=INFO
                )
            )
